...
namespace MonitorPorts
{
    public class Communication
    {
        ...
        {
            IPAddress localIpAddr = IPAddress.Parse("127.0.0.1");       //绑定本机环回地址
        ...
        public void Send()
        {
            while (true)
            {
                ...
                Array.Copy(buffer, sendBuffer, buffer.Length);      //将发送的字符串转为byte数组，并拷贝到sendBuffer中，为了确保每次发送长度为512位
                try
                {
        ...
        public void Check()
        {
            while (true)
            {
                Thread.Sleep(4000);     //每隔四秒判定一次
                if (IsRunning)
                {
                    if (this.ReceiveTime.AddSeconds(12) < DateTime.Now)
                    {
                        Restart();
                        IsRunning = false;
                    }
        ...
        public void Restart()
        {
            ...
            Thread.Sleep(2000);
            this.ReceiveTime = DateTime.Now.AddSeconds(2);                        //更新接收时间
